...
{
    [SerializeField] LoadJson loadJson;
    ...
    {
        foreach (User user in loadJson.userData)
        {
        ...
        foreach (Item item in loadJson.itemData)
        {
    ...
    private void ViewInfoUser(User user)
    {
        ...
        List<Inventory> inventories = loadJson.inventoryData.Where(inventory => inventory.userID == user.userID).ToList();
        foreach (var inventory in inventories)
        {
            GameObject icon = Instantiate(viewImage, infoView.transform);

            // 아이템 탐색
            Item item = loadJson.itemData.FirstOrDefault(item => item.itemID == inventory.itemID);
            if (item != null)
            {
